...
export default function ToolSettings({ activeTool, onSetTool, brushSize, onBrushSize, color, onColor, activeLayerId, regionName, onRegionName, inline }) {
  ...
  // inline = rendered inside a tab, no wrapper needed
  if (inline) {
    return (
      <div className="flex flex-col">
        <div className="px-3 py-2 border-b border-slate-700">
          <p className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Active: {activeLayerId}</p>
        </div>
        <ToolSettingsBody {...{ activeTool, onSetTool, brushSize, onBrushSize, color, onColor, activeLayerId, regionName, onRegionName, TOOLS, showPalette, palette, layerDef }} />
      </div>
    );
  }
...
// Shared body used by inline mode — extracted to avoid duplication
function ToolSettingsBody({ activeTool, onSetTool, brushSize, onBrushSize, color, onColor, activeLayerId, regionName, onRegionName, TOOLS, showPalette, palette, layerDef }) {
  return (
    <>
      <div className="p-2 border-b border-slate-700">
        <div className="grid grid-cols-5 gap-1">
      ...
      {showPalette && (
        <div className="p-3 border-b border-slate-700 space-y-1">
          <label className="text-[10px] text-slate-400">
            {activeLayerId === 'climates' ? 'Climate Zones'
              : activeLayerId === 'features' ? 'Feature Colors'
              : activeLayerId === 'heights' ? 'Height Presets'
              : 'Ground Types'}
          </label>
