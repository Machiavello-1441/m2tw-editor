...
// Religions editor — sliders keep the sum at exactly 100. Moving one
// religion redistributes the delta proportionally across the others.
function ReligionsEditor({ religions, availableReligions, onChange }) {
  ...
  // Rescale `others` so the sum of all religions equals 100, given that
  // `key` now has `newVal`. Distributes proportionally to each one's current
  // share; rounding drift is dumped onto the largest other to keep sum exact.
  const rescale = (r, key, newVal) => {
    const keys = Object.keys(r);
    if (keys.length === 0) return r;
    const others = keys.filter(k => k !== key);
    if (others.length === 0) {
      // single religion — forced to 100
      r[key] = 100;
      return r;
    }
  ...
  const remove = (key) => {
    ...
    delete next[key];
    // Redistribute the removed share proportionally across the rest
      ...
      {Object.entries(religions || {}).map(([rel, val]) => (
        <div key={rel} className="space-y-0.5">
          <div className="flex items-center gap-1">
            ...
            <input
              type="range" min={0} max={100} step={1} value={parseInt(val) || 0}
