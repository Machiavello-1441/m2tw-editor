...
import { Plus, Trash2, GripVertical, Swords, Shield } from 'lucide-react';
import { BUILDING_TRAITS, FACTIONS, CULTURES } from './EDBParser';
import RequirementBuilder from './RequirementBuilder';

function RecruitPoolRow({ cap, index, onChange, onRemove }) {
  const [showReqs, setShowReqs] = useState(false);
  
  return (
    <div className="bg-accent/50 rounded-lg p-3 space-y-2">
      <div className="flex items-start gap-2">
        <Swords className="w-3.5 h-3.5 text-primary mt-2 shrink-0" />
        <div className="flex-1 grid grid-cols-2 lg:grid-cols-5 gap-2">
          <Input
            className="h-7 text-xs col-span-2 lg:col-span-1"
          ...
          <Input
            ...
            onChange={e => onChange(index, { ...cap, experience: parseInt(e.target.value) || 0 })}
          />
        </div>
...
export default function CapabilityEditor({ capabilities, onChange }) {
      ...
      {recruitPools.length > 0 && (
        <div className="space-y-2">
          ...
          {recruitPools.map((cap, i) => {
            const realIndex = capabilities.indexOf(cap);
            return <RecruitPoolRow key={i} cap={cap} index={realIndex} onChange={handleChange} onRemove={handleRemove} />;
          })}
        </div>
      )}
