  ...
  const handleFolderLoad = (e) => {
    ...
    files.forEach(file => {
      ...
      reader.onload = (ev) => {
        ...
        newLoaded[loaderKey] = true;
        count++;
        if (count === Object.keys(newLoaded).length) {
          setLoaded(p => ({
            ...p,
            fac: newLoaded['loadFactionsFile'] || p.fac,
            res: newLoaded['loadResourcesFile'] || p.res,
            ev: newLoaded['loadEventsFile'] || p.ev,
            unit: newLoaded['loadUnitsFile'] || p.unit,
          }));
        }
      };
      reader.readAsText(file);
